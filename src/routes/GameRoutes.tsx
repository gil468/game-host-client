...
const GameRoutes = () => {
    ...
    addEvent({eventName : 'round-started', callback : (x :  [{ songId: number; }]) => startGame(x[0].songId),newStatus : 'Running', stateArray : ['WaitingRoom','BetweenRounds'] })

        addEvent({eventName : 'playerJoined', callback : (player) => {
          setWaitingPlayers(x => ([...x, player[0].userName]))
        }, newStatus : 'WaitingRoom', stateArray : ['WaitingRoom'] })

        addEvent({eventName : 'buzzerGranted', callback : () => {},
         newStatus : 'Buzzered', stateArray : ['Running'] })

         addEvent({eventName : 'correctAnswer', callback : (x) => {
          ...
          newStatus :'BetweenRounds', stateArray : ['Buzzered'] })

         addEvent({eventName : 'wrongAnswer', callback : (x) => {
